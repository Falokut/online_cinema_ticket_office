...
import (
	...
	mock_repository "github.com/Falokut/online_cinema_ticket_office/image_storage_service/internal/repository/mocks"
...
func newClient(t *testing.T, s *service.ImageStorageService) *grpc.ClientConn {
	return newServer(t, func(srv *grpc.Server) { protos.RegisterImageStorageServiceV1Server(srv, s) })
}
...
func TestGetImage(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		conn := newClient(t, service.NewImageStorageService(logger,
			service.Config{}, repo))
		defer conn.Close()

		client := protos.NewImageStorageServiceV1Client(conn)
...
func TestDeleteImage(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		conn := newClient(t, service.NewImageStorageService(logger, service.Config{}, repo))
		defer conn.Close()
...
func TestReplaceImage(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		conn := newClient(t, service.NewImageStorageService(logger, service.Config{MaxImageSize: testCase.maxImageSize}, repo))
		defer conn.Close()
